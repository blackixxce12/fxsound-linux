...
impl<'model> RnnState<'model> {
    ...
    // fxsound: zero the three GRU states in place. `DenoiseState::new` builds these with `vec!`,
    // so before this the only way to start the network afresh was to allocate a new one.
